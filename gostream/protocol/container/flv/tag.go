...
func (tag *Tag) IsKeyFrame() bool {
    return tag.mediaTag.frameType == FrameTypeKEY
}

func (tag *Tag) IsSeq() bool {
    return tag.mediaTag.frameType == FrameTypeKEY &&
        tag.mediaTag.avcPacketType == AVCPacketTypeSeqHeader
}
...
func (tag *Tag) parseAudioHeader(b []byte) (n int, err error) {
    ...
    flags := b[0]
    tag.mediaTag.soundFormat = flags >> 4
...
func (tag *Tag) parseVideoHeader(b []byte) (n int, err error) {
    ...
    flags := b[0]
    tag.mediaTag.frameType = flags >> 4
    tag.mediaTag.codecID = flags & 0xf
    n++
    if tag.mediaTag.frameType == FrameTypeInter || tag.mediaTag.frameType == FrameTypeKEY {
