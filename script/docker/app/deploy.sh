...
#启动基础模块
base(){
	docker-compose up -d nacos sentinel seata-server web-nginx blade-nginx blade-redis minio mysql hbase
}
...
modules(){
	...
	docker-compose up -d blade-gateway1  blade-auth1  blade-user blade-desk blade-system blade-log blade-resource
}
emgrp(){
	docker-compose up -d emgrp-task emgrp-uav emgrp-data emgrp-person gostream
}
#关闭所有模块
stop(){
	docker-compose stop
